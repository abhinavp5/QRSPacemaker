...
def generate_sample_ecg(duration_seconds=60, sample_rate=360):
    ...
    for i in range(int(duration_seconds / beat_period)):
        beat_start = i * beat_period
        
        # P wave (small positive deflection)
        p_center = beat_start + 0.1
        p_mask = np.abs(t - p_center) < 0.05
        ecg[p_mask] += 0.2 * np.exp(-((t[p_mask] - p_center) / 0.02)**2)
        
        # QRS complex (main spike)
        qrs_center = beat_start + 0.25
        qrs_mask = np.abs(t - qrs_center) < 0.08
        ecg[qrs_mask] += 2.0 * np.exp(-((t[qrs_mask] - qrs_center) / 0.03)**2)
        
        # T wave (positive deflection after QRS)
        t_center = beat_start + 0.5
        t_mask = np.abs(t - t_center) < 0.1
        ecg[t_mask] += 0.3 * np.exp(-((t[t_mask] - t_center) / 0.05)**2)
    
    # Add some noise
    ecg += 0.1 * np.random.normal(0, 1, len(t))
    
    return ecg.tolist()

# Generate synthetic ECG data
ecg_samples = generate_sample_ecg(duration_seconds=120)
